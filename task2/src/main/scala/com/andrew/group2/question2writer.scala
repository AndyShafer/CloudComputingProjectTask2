...
class Question2Writer extends ForeachWriter[Row] {
  private val tableName = "2_2"
  private val regionName = "us-east-1"
  private var currentAirport = ""
  private var destinations : Seq[String] = Seq()
  ...
  def submit() = {
    if(currentAirport != "") {
      val rowMap = Map("airport" -> currentAirport, "top_10_destinations" -> destinations.toString)
  ...
  def process(row: Row) = {
    ...
    if(currentAirport == airport) {
      if(destinations.size < 10) {
        destinations = destinations :+ row.getString(1)
      }
    } else {
      submit()
      currentAirport = row.getString(0)
      destinations = Seq(row.getString(1))
    }
  }
